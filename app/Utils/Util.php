...
class Util
{
    ...
    */
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        $payment_types = ['cash' => __('lang_v1.cash'), 'card' => __('lang_v1.card'),'mpesa' => __('M-PESA'), 'cheque' => __('lang_v1.cheque'), 'bank_transfer' => __('lang_v1.bank_transfer'), 'other' => __('lang_v1.other')];
    ...
    public function replaceTags($business_id, $data, $transaction, $contact = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{contact_name}') !== false) {
              ...
              //Replace loyalty points
            if (strpos($value, '{loyalty_points}') !== false) {
                $loyalty_points = empty($contact) ? $transaction->contact->total_rp : $contact->total_rp;
                $data[$key] = str_replace('{loyalty_points}', $loyalty_points, $data[$key]);
            }

             //Replace loyalty points
            if (strpos($value, '{advance_balance}') !== false) {
                $advance_balance = empty($contact) ? $transaction->contact->balance : $contact->balance;
                $data[$key] = str_replace('{advance_balance}', $advance_balance, $data[$key]);
            }
    ...
    */
    ...
    {
        $user_details = $request->only(['surname', 'first_name', 'last_name', 'email',
                    ...
                    'custom_field_2', 'custom_field_3', 'custom_field_4', 'guardian_name', 'id_proof_name'
                    , 'id_proof_number', 'permanent_address', 'current_address', 'bank_details','selected_contacts']);
