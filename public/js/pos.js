...
$(document).ready(function () {
    ...
    $('button.pos-express-finalize').click(function () {
        // console.log('The final button was clicked');
        //Check if product is present or not.
        if ($('table#pos_table tbody').find('.product_row').length <= 0) {
            toastr.warning(LANG.no_products_added);
            return false;
        ...
        // console.log('Modal:', $('div#mpesa_details_modal'));
        ...
        if (pay_method == 'card') {
            ...
            $('#total_amount').val($('#final_total_input').val());
            $('#total_amount').prop('disabled', true);
            $('div#mpesa_details_modal').modal('show');
        } else {
            pos_form_obj.submit();
        }
    });
    ...
    $('div#mpesa_details_modal').on('shown.bs.modal', function (e) {
    ...
    $('#pos-save').click(function () {
        var pay_method = $('.payment_types_dropdown').val();
        if (pay_method == 'mpesa') {
            console.log('doing the mpesa thing');
            var phone_number = $('#mpesa_phonenumber').val();
            var total_amount = $('#final_total_input').val();
            var cash_amount = $('#mpesa_cashamount').val();
            ...
            initializeStkPush(data);
        } else {
            pos_form_obj.submit();
        }
    });

    //on save M-PESA details
    $('button#pos-save-mpesa').click(function () {
    ...
    function initializeStkPush(data) {
        toastr.success('Initiating papyment...');
        $.ajax({
            method: 'POST',
            url: '/api/v1/stk/push',
            data: {
        ...
        function checkPaymentStatus() {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    console.log('M-PESA data:', result);
                    console.log('Trials:', tries);
                    if (result.status == 'success') {
                        console.log(result);
                        console.log('Latest Update Nov1 12:48');
                        // var formattedAmount = parseInt(data.total_amount);
                        // var formattedCash = parseInt(data.cash_amount);
                        // console.log('Formatted Amount:', formattedAmount);
                        // if (result.data.total_amount == formattedAmount - formattedCash) {
                        // if (result.status == 'success') {
                        toastr.success(
                            `Payment of ${result.data.total_amount} succesfully recived.`
                        );
                        $('div#mpesa_details_modal').modal('hide');
                        if (data.cash_amount > 0) {
                            $('.payment_types_dropdown').val('cash');
                            $('#amount_0').val(data.cash_amount);
                            $('[name="final_total"]').val(data.cash_amount);
                            $('#mpesa_phonenumber').val(data.phone_number);
                            pos_form_obj.submit();
                            pos_form_mpesa = $('form#add_pos_sell_form');
                            $('.payment_types_dropdown').val('mpesa');
                            $('[name="final_total"]').val(result.data.total_amount);
                            $('#amount_0').val(result.data.total_amount);
                            $('#mpesa_phonenumber').val(data.phone_number);
                            pos_form_mpesa.submit();
                        ...
                        if (tries >= 10) {
                            toastr.error('Payment not received, cancelling transaction...');
                            $.ajax({
                                type: 'POST',
                                url: '/api/v1/mpesa/transaction/cancel',
                                data: {
                                    phone: data.phone_number,
                                    amount: data.total_amount,
                                    cash: data.cash_amount,
        ...
        function completePayment(data) {
            $.ajax({
                type: 'POST',
                url: '/api/v1/mpesa/transaction/complete',
                data: {
...
function set_payment_type_dropdown() {
    ...
    if (enabled_payment_types.length) {
        $('.payment_types_dropdown > option').each(function () {
            //skip if advance
            if (($(this).val() && $(this).val() != 'advance') || $(this).val() != 'mpesa') {
...
function pos_print(receipt) {
    ...
    if (receipt.print_type == 'printer') {
        ...
        // var randoString = genRand(5);
        // window.location.href = `/pos/generatepdf/${randoString}`;
        //     //     return false
        //    $.ajax({
        //        type: 'POST',
        //        url: `/pos/generatepdf/${randoString}`,
...
$(document).on('change', '.payment_types_dropdown', function (e) {
    ...
    if (payment_type && payment_type != 'advance') {
        ...
        // }
        console.log('default accounts:', default_accounts);
